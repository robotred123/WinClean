...
namespace Clean_Recycle
{
    public partial class Frm_Dm_CB : Form
    {
        ...
        private void Frm_Dm_CB_Load(object sender, EventArgs e)
        {
            ...
            { cb_dvc1.Text = dt_dmcb.Rows[0]["dv_cap1"].ToString(); cb_dvc2.Text = dt_dmcb.Rows[0]["dv_cap2"].ToString(); cb_dvc3.Text = dt_dmcb.Rows[0]["dv_cap3"].ToString(); cb_dvc4.Text = dt_dmcb.Rows[0]["dv_cap4"].ToString(); txt_hoten.Text = dt_dmcb.Rows[0]["Hoten"].ToString(); txt_chucvu.Text = dt_dmcb.Rows[0]["Position"].ToString(); _cbStartWithWindows.Checked = Convert.ToBoolean(dt_dmcb.Rows[0]["Auto_start"].ToString()); }
        }
